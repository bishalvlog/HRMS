...
namespace HRMS.Services.Http
{
    public abstract class BaseHttpClient : IBaseHttpClient
    {
        ...
        protected virtual HttpClient SetHeaders(HttpClient httpClient, IEnumerable<KeyValuePair<string,string>> headers)
        {
            ...
            {
                if(httpClient.DefaultRequestHeaders.TryGetValues(headersKv.Key, out var _))
                    httpClient.DefaultRequestHeaders.Remove(headersKv.Key);

                httpClient.DefaultRequestHeaders.Add(headersKv.Key,headersKv.Value);
            }
            return httpClient;
        }
